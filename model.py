# -*- coding: utf-8 -*-
"""
Created on Sun Mar 11 14:49:03 2018
...
def get_embedding():
    embeddings_index = {}
    f = open(EMBEDDING_FILE,encoding = "ISO-8859-1")
    for line in f:
        values = line.split()
        word = values[0]
...
train = pd.read_csv("trainpanda.csv",encoding = "ISO-8859-1")
test = pd.read_csv("train_test.csv",encoding = "ISO-8859-1")
...
test_nlp_features = pd.read_csv("nlp_features_test.csv",encoding = "ISO-8859-1")
test_non_nlp_features = pd.read_csv("non_nlp_features_test.csv",encoding = "ISO-8859-1")
...
for idx_train, idx_val in skf.split(train["is_duplicate"], train["is_duplicate"]):
    ...
    submission['q1']=test["question1"]
    submission['q2']=test["question2"]
    submission.to_csv("predictions/" + str(model_count) + ".csv", index=False)
